    ...
    def compose_conversation(
        ...
        for round_index, conversation_round in enumerate(rounds):
            for post_index, post in enumerate(conversation_round.post_list):
                ...
                user_message = ""
                assistant_message = ""
                is_last_post = round_index == len(rounds) - 1 and post_index == len(conversation_round.post_list) - 1
                if is_first_post:
                    user_message = (
                ...
                if len(user_message) > 0:
                    ...
                    if is_last_post and add_requirements:
...
def format_code_feedback(post: Post) -> str:
    ...
    for attachment in post.attachment_list:
        if attachment.type == "verification" and attachment.content == "CORRECT":
            feedback += "## Verification\nI have verified that your code is CORRECT.\n"
            verification_status = "CORRECT"
        elif attachment.type == "verification" and attachment.content == "NONE":
            feedback += "## Verification\nNo code verification.\n"
            verification_status = "NONE"
        elif attachment.type == "verification" and attachment.content == "INCORRECT":
            feedback += "## Verification\nYour code is INCORRECT with the following error:\n"
            verification_status = "INCORRECT"
        elif attachment.type == "code_error" and verification_status == "INCORRECT":
            feedback += f"{attachment.content}\n"
        elif attachment.type == "execution_status" and attachment.content == "NONE":
            feedback += "## Execution\nNo code execution.\n"
            execution_status = "NONE"
        elif attachment.type == "execution_status" and attachment.content == "SUCCESS":
            feedback += "## Execution\nYour code has been executed successfully with the following result:\n"
            execution_status = "SUCCESS"
        elif attachment.type == "execution_status" and attachment.content == "FAILURE":
            feedback += "## Execution\nYour code has failed to execute with the following error:\n"
            execution_status = "FAILURE"
        elif attachment.type == "execution_result" and execution_status != "NONE":
            feedback += f"{attachment.content}\n"
    return feedback
