...
from .base import CompletionService, EmbeddingService, LLMServiceConfig,PlannerServiceConfig,CodeInterpreterServiceConfig
    ...
    def __init__(self, config: OpenAIServiceConfig):
        ...
        self.client: OpenAI = (
            ...
            if api_type == "openai"
                ...
                api_key=(self.config.api_key if api_type ==
                         "azure" else self._get_aad_token()),
            )
        )
...
class OpenAIPlannerServiceConfig(PlannerServiceConfig):
    def _configure(self) -> None:
        self._set_name("openai")

        # shared common config
        self.api_type = self.planner_config.api_type
        assert self.api_type in ["openai", "azure",
                                 "azure_ad"], "Invalid API type"

        shared_api_base = self.planner_config.api_base
        self.api_base = self._get_str(
            "api_base",
            shared_api_base if shared_api_base is not None else "https://api.openai.com/v1",
        )
        shared_api_key = self.planner_config.api_key
        self.api_key = self._get_str(
            "api_key",
            shared_api_key if shared_api_key is not None else (
                "" if self.api_type == "azure_ad" else None),
        ...
        shared_model = self.planner_config.model
        self.model = self._get_str(
            "model",
            shared_model if shared_model is not None else "gpt-4",
        )
        shared_backup_model = self.planner_config.backup_model
        self.backup_model = self._get_str(
            "backup_model",
            shared_backup_model if shared_backup_model is not None else self.model,
        )
        shared_embedding_model = self.planner_config.embedding_model
        self.embedding_model = self._get_str(
            "embedding_model",
            shared_embedding_model if shared_embedding_model is not None else "text-embedding-ada-002",
        )

        self.response_format = self.planner_config.response_format
...
class OpenAIPlannerService(CompletionService, EmbeddingService):
    @inject
    def __init__(self, config: OpenAIPlannerServiceConfig):
        self.config = config
...
class OpenAICodeInterpreterServiceConfig(CodeInterpreterServiceConfig):
    def _configure(self) -> None:
        self._set_name("openai")

        # shared common config
        self.api_type = self.code_interpreter_config.api_type
        assert self.api_type in ["openai", "azure",
                                 "azure_ad"], "Invalid API type"

        shared_api_base = self.code_interpreter_config.api_base
        self.api_base = self._get_str(
            "api_base",
            shared_api_base if shared_api_base is not None else "https://api.openai.com/v1",
        )
        shared_api_key = self.code_interpreter_config.api_key
        self.api_key = self._get_str(
            "api_key",
            shared_api_key if shared_api_key is not None else (
                "" if self.api_type == "azure_ad" else None),
        ...
        shared_model = self.code_interpreter_config.model
        self.model = self._get_str(
            "model",
            shared_model if shared_model is not None else "gpt-4",
        )
        shared_backup_model = self.code_interpreter_config.backup_model
        self.backup_model = self._get_str(
            "backup_model",
            shared_backup_model if shared_backup_model is not None else self.model,
        )
        shared_embedding_model = self.code_interpreter_config.embedding_model
        self.embedding_model = self._get_str(
            "embedding_model",
            shared_embedding_model if shared_embedding_model is not None else "text-embedding-ada-002",
        )

        self.response_format = self.code_interpreter_config.response_format
...
class OpenAICodeInterpreterService(CompletionService, EmbeddingService):
    @inject
    def __init__(self, config: OpenAICodeInterpreterServiceConfig):
        self.config = config
